package com.example.ands_hw2_apk;
...
public class Activity_Menu extends AppCompatActivity {
    ...
    private void initViews() {
        this.menu_BTN_start.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                String tempID = Activity_Menu.this.menu_EDT_id.getText().toString();
                if (tempID.length() < 8) {
                    Toast.makeText(Activity_Menu.this, "Please enter an ID with at least 8 characters", Toast.LENGTH_SHORT).show();
                } else if (tempID.length() > 9) {
                    Toast.makeText(Activity_Menu.this, "The ID must be 9 digits", Toast.LENGTH_SHORT).show();
    ...
    public void makeServerCall() {
        new Thread() {
            public void run() {
                ...
                Log.d("pttt", Activity_Menu.this.getString(R.string.url) + "");
                Log.d("pttt", data);
                if (data != null) {
    ...
    public void startGame(String id, String data) {
        ...
        String state = temp[Integer.parseInt(String.valueOf(id.charAt(7)))];
